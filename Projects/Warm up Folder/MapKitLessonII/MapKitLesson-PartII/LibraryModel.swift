...
class LibraryWrapper: NSObject,Codable, MKAnnotation {
    ...
    @objc var coordinate: CLLocationCoordinate2D {
        let latLong = position.components(separatedBy: ",")
            .map{$0.trimmingCharacters(in: .whitespacesAndNewlines) }
            .map({Double($0)})
        guard latLong.count == 2, let lat = latLong[0], let long = latLong[1] else {return CLLocationCoordinate2D()}
...
//to conform to MKAnnotation, has to also conform to NSOBject
